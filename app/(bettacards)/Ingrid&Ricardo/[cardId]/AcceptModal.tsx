...
const AcceptModal= () => {
  ...
  const sendConfirmationMessage = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    if(confirmation.length <= 6 ) return setError('Escribe más:)')
  ...
  return (
    <div className='bg-th-primary-dark text-th-primary-light text-5xl p-20 flex flex-col rounded-lg'>
      ...
      {isSending 
        ? <p className='mt-5 rounded-md p-3 w-2/3 self-center text-th-accent-light text-6xl'>Enviando <span className='animate-ping'>...</span></p>
        : <button
            disabled={success}
